...
| 17 | `git-workflow-and-branching` | **Land** | 已覆盖 | trunk-based 等已由 finishing-branch 覆盖 |
| 18 | `ci-cd-and-automation` | **Land** | 不吸收 | 各项目 CI 差异大，通用 skill 用处有限 |
| 19 | `shipping-and-launch` | **Land** | 已覆盖 | finishing-branch + Gate 体系已更强 |
...
| `brainstorming` | superpowers | **子调用复用**: `Skill(superpowers:brainstorming)`，方案探索时调用 |
...
| 集成 Skill | 来源 | 集成方式 |
|---|---|---|
| `finishing-a-development-branch` | superpowers | **直接复用**: + rule-finishing-branch overlay + Gate 体系 |
| `git-workflow-and-branching` | agent-skills | **已覆盖**: trunk-based 等由 finishing-branch 覆盖 |
