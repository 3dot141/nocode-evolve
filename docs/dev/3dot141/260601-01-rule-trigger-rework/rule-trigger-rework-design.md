...
**主因假设（最痛，待验证）：触发率瓶颈可能在"深度负载下的遵守失败"，而非"组织失败"。** 单点 evidence —— `docs/dev/3dot141/260526-02-rule-trigger-eval/rule-trigger-eval-design.md:183` 记录 `finishing-branch` 在 clean-room route-recall **6/6 满分**、真实主会话仍触发失败，提示"组织 / 路由描述写好了也未必保证遵守"。
