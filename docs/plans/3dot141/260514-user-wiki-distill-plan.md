...
**Goal:** 把 `docs/plans/3dot141/260514-user-wiki-distill-design.md` 的设计落地为可用的 `/user-wiki-distill` slash command——一份 command markdown + 一份 python 脚本 + plugin.json 版本号。
...
**Spec 引用：** `docs/plans/3dot141/260514-user-wiki-distill-design.md`——本 plan 的所有契约、字段顺序、exit code、stdout 协议都以 spec 为准；本 plan 不重述设计原理，只给"怎么写代码"。
...
Plan 完成并保存到 `docs/plans/3dot141/260514-user-wiki-distill-plan.md`。两种执行方式选一：
