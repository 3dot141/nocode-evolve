...
```
Expected: `finishing-branch` route-recall 不低于改造前基线（`docs/plans/3dot141/260526-rule-trigger-eval-design.md:183` 记的 6/6）；`git-worktree` 召回不退步。若粗桶改造导致退步 → 回 Task 6 调桶触发/负例文案。
...
Plan 已保存到 `docs/plans/3dot141/260601-rule-trigger-rework-plan.md`，9 个 task / 4 phase。两种执行方式：
