...
| 非 `null` 且 `commitDiff`/`dirtyFiles` 均为空 | `false` | **秒回**，不进入 Phase 1，直接输出：`✓ plugin-dream：自上次检查以来 rules/skills/commands/hooks/scripts/.claude-plugin/plugin.json 无变化，无需维护`，命令结束 |
