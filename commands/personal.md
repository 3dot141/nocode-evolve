---
description: .agents-personal/ 统一管理入口，分发到 7 个子动作（init/write/search/check/tidy/snap/status）
argument-hint: <sub-action> [args]
---

# /personal：.agents-personal/ 管理入口

统一入口，分发到 7 个子动作。每个子动作也可以直接用独立命令调用。

## 用法

`/personal <sub-action> [args]`
...
| `write` | 写入 wiki + rules + AGENTS.md | `Skill(nocode-evolve:personal-distill)` | `/personal-distill` |
| `search` | 检索 .agents-personal/ 内容 | `Skill(nocode-evolve:personal-recall)` | `/personal-recall` |
...
1. 检查 `.agents-personal/` 是否存在 → 不存在报 "未初始化，跑 `/personal init`"
2. 统计：
