...
1. **转发到 `Skill(nocode:xxx)`**——绝大多数 hub / 操作型 / 巡检型命令走这条（`distill.md`/`personalhub.md`/`projecthub.md`/`nocodehub.md`/`plugin-dream.md`/`plugin-distill.md`/`personal-dream.md`/`personal-distill.md` 等文件里都能看到 `Skill(...)` 调用）。
