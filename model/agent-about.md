...
- 形态：解释控制流 / UI 结构 / 数据流 / 文件布局 / 变化点时，优先使用对应的紧凑视觉形式（伪代码块、调用树、组件树、浅层文件树、时序图、diff 形态树图），而不是连续散文——选能讲清当前要点的最小视图，紧贴它支撑的文字。形式选择映射见 `{NOCODE_SKILL_REF}/visual-forms.md`；连续散文超过一个要点时，先停下来判断能否转图示。
