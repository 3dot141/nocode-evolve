...
#### pd-vd
**触发**: 用户说「视觉设计 / 视觉方向 / 配色 / 原型 / wireframe / 线框图 / 设计稿 / 长什么样 / 出个原型」, 或 pd-ix 完成后衔接, 或 pdflow 在交互设计后路由到视觉设计阶段
**读**: ``
**也属**: workflow
**生命周期**: 0 设计
...
**摘要**: 本机 Codex 当独立模型接四场景 (红蓝独立审查 / 代码 review 收尾 / 委派救援 / 设计文档审稿); 探活 (setup --json) 主 agent 直接 Bash, 真正的 review/adversarial-review/task 调用派 subagent 执行 (Agent() 包一层 Bash, 不在主 agent 直接 Bash 调 vendor/codex/scripts/codex-companion.mjs); 场景 1/4 默认单跑 Codex, 探活不通过才 fallback 改派独立 subagent (不再并行双跑); 不可用降级自做 + 明说; 禁改 vendored 文件
**主桶**: review (完整定义见该桶)
