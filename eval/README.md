...
> 已知漂移：`eval/cases/dev-design.md` 已使用当前合并后的 Skill 名；其它历史 fixture 仍可能使用旧短 id。用之前先核对
...
3. 如需新的情境铺垫，在 `eval/preambles/` 加一份，并在 fixture 的 `preamble_profiles`
   里登记。
4. 当前 fixture 包含 build / dev-design / finish-branch / plan / verify；给新规则或改动较大的路由补 fixture 是欢迎的。
