...
src/components/
  update_plan/
    update_plan.tsx          # Component implementation
    update_plan.test.tsx     # Tests
    update_plan.stories.tsx  # Storybook stories (if using)
...
<Card
  ...
  headerVariant="large"
  bodyPadding="md"
  content={<update_plan tasks={tasks} />}
/>
```
...
export function TaskListContainer() {
  ...
  return <update_plan tasks={tasks} />;
}

// Presentation: handles rendering
export function update_plan({ tasks }: { tasks: Task[] }) {
  return (
...
function update_plan({ tasks }: { tasks: Task[] }) {
  if (tasks.length === 0) {
    return (
