...
`node "${PLUGIN_ROOT}/scripts/xxx.mjs"`（CLI）或 `import()`（library）两种方式消费。
...
| `personal-lint.mjs` | `commands/personal-lint.md`；`commands/distill.md` / `commands/personal-distill.md` / `commands/personal-dream.md` / `commands/personalhub.md` 均通过 `$personal-lint` 间接触发 | CLI + Skill 包装 |
