...
```bash
node -e "
import('${PLUGIN_ROOT}/scripts/personal-snapshot.mjs').then(async ({ resolvePersonalDir }) => {
  const { diffSinceBaseline } = await import('${PLUGIN_ROOT}/scripts/dream-baseline.mjs');
  const { snapshot } = await import('${PLUGIN_ROOT}/scripts/personal-snapshot.mjs');
  const personalDir = resolvePersonalDir(process.env.cwd || process.cwd());
...
对候选范围每页：
1. 用 `node "${PLUGIN_ROOT}/scripts/wiki-read.mjs" --project-root "$PWD" --path "<wiki-page-path>" --session-id "<current-session-id>"` 读页面及 frontmatter（created、last_updated、maturity、related、sources）
...
```bash
node -e "
import('${PLUGIN_ROOT}/scripts/dream-baseline.mjs').then(async ({ advanceBaseline }) => {
  const { resolvePersonalDir } = await import('${PLUGIN_ROOT}/scripts/personal-snapshot.mjs');
  const personalDir = resolvePersonalDir(process.env.cwd || process.cwd());
