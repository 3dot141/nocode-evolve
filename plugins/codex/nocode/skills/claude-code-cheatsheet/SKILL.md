...
| 工具 | 干嘛 | 关键点 |
|---|---|---|
| `update_plan` / `update_plan` / `update_plan` / `update_plan` | 会话内结构化任务清单，跟踪多步进度 | v2.1.142 起取代 `TodoWrite`；进 workflow skill 要求 Step 0 先把全部 task 建出来 |
...
| 工具 | 干嘛 | 关键点 |
|---|---|---|
| `Codex workdir` / `return to the main workdir` | 建隔离 git worktree 并把会话切进去 | **只在用户或 CLAUDE.md 明确要 worktree 时才用**；`path` 进已存在的 worktree、`name` 建新的 |
...
| `request_user_input` | 结构化选项问用户 | 待确认内容要写进 payload 自足，别指代前文 |
