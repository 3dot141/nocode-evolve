...
3. **展示 + 确认（拆两回合，大纲/骨架禁塞 request_user_input）**：
   ...
   - **确认回合**：用户回应通常已是决策（确认 / 具体调整意见）→ 直接采纳，不再补 ask；回应含糊才 request_user_input 澄清。要调整 → 改后重走展示回合；章节太多 → 去掉不需要的（小改动不需监控/eval 章）。
...
- 都没改（接口参数 / 命名 / 模块内部实现）→ **局部决策**：writing 自己定，小问题 request_user_input 当场确认，不中断
...
**升审只在两种情况**：
- **用户显式要求**（「审一下 / 深审 / 独立审 / 找 codex」）→ 调 `$reviewing`，声明：对象 = 设计文档；领域维度 = design-doc-review 8 维；Context Capsule = 已拍板决策 / 被否决方案及原因 / 非目标 / 预算（不带作者预期结论）——派发 / CLAIM 剥离 / 降级由引擎承载
...
2. **request_user_input：是否渲染成 Artifact 页面？**
   - 是 → 进入 render 阶段（Read `${PLUGIN_ROOT}/shared/references/doc-render.md` 执行），把设计文档渲染成 Artifact 页面（图用 DOM 排版、表格可交互、可分享 URL）
...
- ❌ 因"任务简单 / 还在概览 / 用户说了'继续'"跳过某 Step、不建 Step 0 update_plan、或漏掉收尾交接——进了流程就走完所有 Step
...
- `${PLUGIN_ROOT}/shared/references/doc-render.md` — 已定稿 markdown 文档 → Artifact 页面（共享 reference，260716 从 dev-design 内部抽出）
