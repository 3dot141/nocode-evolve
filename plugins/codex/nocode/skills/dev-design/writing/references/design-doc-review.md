...
>
> **怎么用**：dev-design writing 的 Review step Read 本文拿维度，**默认主会话就地逐维自查**（不调 reviewing 引擎、不派 subagent/Codex）；用户显式要求升审时才调 `$reviewing`，声明「对象 = 设计文档、方法 = checklist、领域维度 = 本文 8 维度 + 附带检查」（此时 reviewer 纪律 / Evidence Gate / 分级 schema 由引擎自带）。自查时的编号（C/W/S/Q/SA）与 Evidence 纪律见 SKILL.md Review 节。
>
