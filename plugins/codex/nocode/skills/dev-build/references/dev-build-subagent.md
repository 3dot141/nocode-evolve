...
开始前 Read `${PLUGIN_ROOT}/skills/references/design-traceability.md`。派发 objective 必须原样包含 Plan 的 `designRevision` / `designDigest` 与当前 task 的 `designCovers`，并要求 result 返回相同基线、`completedDesignCovers`、`changedFiles` 和测试 `evidence`；implementer 不得自行改设计基线或 ID 覆盖范围。
