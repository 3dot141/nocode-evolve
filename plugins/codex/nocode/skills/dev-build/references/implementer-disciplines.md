...
- **Source check**：Read 所有涉及代码/文档，标注 `[Read path:line]` / `[Doc URL]` / `[推断]`。查文档按优先级：① `[provider-neutral skill boundary]` / `[provider-neutral skill boundary]` 查库/框架文档与 API；② `mcp__exa__web_search_exa` / `mcp__exa__get_code_context_exa`；③ `[provider-neutral skill boundary]` 直查目标 GitHub repo 文档。反模式："这个 API 应该是这样用的"——没标来源就当事实写下去；把旧版本文档行为当当前版本；复制网上代码片段不核对对应版本
