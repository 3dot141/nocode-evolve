...
description: "Use when explicitly starting, stopping, restarting, or checking the local fx-data-agents web/ag…"
---
...
```text
${PLUGIN_ROOT}/skills/agents-launcher/dev-orchestrator.mjs
```
...
例外（260901 起）：web 域在编排层自动处置归属——`:10001` 已被**本 `FX_WEB_DIR`** 的 vite 持有时自动复用不重起（无需询问）；被外部进程持有时启动直接失败并给出 pid 与处置指引。skill 层不必再替 web 询问保留/重启，看到 `复用已运行实例` 日志即为保留。
