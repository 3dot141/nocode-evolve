---
name: project-lint
description: "检查项目子目录 AGENTS.md 和 README.md 的健康状态（stale 引用 / 覆盖缺口 / 内容过时）"
---

> Codex 入口：原命令参数统一称为“用户本次调用参数”。

# /project-lint：子目录文档健康检查

检查项目子目录文档的健康状态。

## 入参

`/project-lint [dir-path]`
- 有参数：只检查指定目录
- 无参数：检查整个项目
...
检测方式：扫描项目子目录（同 project-dream 的有意义判断），标记两者都缺的。
