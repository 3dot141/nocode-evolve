...
| 子动作 | 做什么 | 转发到 | 独立命令 |
|---|---|---|---|
| `write` | 为指定目录写入/更新 AGENTS.md + README.md | `$project-distill` | `/project-distill` |
| `dream` | 递归：选定目录，逐层扫子目录，批量生成 | `$project-dream` | `/project-dream` |
| `search` | 搜索项目内所有子目录 AGENTS.md / README.md | `$project-recall` | `/project-recall` |
| `check` | 健康检查（stale 引用 / 覆盖缺口） | `$project-lint` | `/project-lint` |
...
从 `用户本次调用参数` 取第一个词作为子动作，剩余部分作为子动作的参数传递。
