...
**被 `/distill` 调用**（distill 路由 wiki:project / rules:project / agents:project 时 `$personal-distill` 传入候选）。也可用户直接 `/personal-distill` 独立写入。
...
调 `$personal-lint` 做健康检查。结果附在最终报告底部。error 不阻断写入（用户可选先修复或继续），但结论必须明确指出。
