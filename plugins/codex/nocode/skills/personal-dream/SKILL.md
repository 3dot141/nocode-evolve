...
`changedFiles` 非 `null` 时才继续调 `$personal-lint` 获取健康状态，然后对**候选范围**（而不是 `wiki/draft/` + `wiki/pages/` 全部）做深度检查：
...
输出动作表格，用 request_user_input 多选让用户勾选（每个 option 自带「编号 + 动作 + 对象」，不依赖上方表格渲染——工具调用间文本可能被吞）：
...
- **archive**：request_user_input 二选——标 superseded 保留 / 删除。标 superseded 不走删除护栏，删除走
