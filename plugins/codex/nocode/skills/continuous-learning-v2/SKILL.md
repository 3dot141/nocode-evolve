...
description: "Reference doc for the instinct-based continuous learning architecture — PreToolUse/PostToolUse…"
version: 2.0.0
---
...
```
${PLUGIN_ROOT}/skills/continuous-learning-v2/hooks/observe.sh
```

Claude Code expands `${PLUGIN_ROOT}` to the real plugin install directory when it loads `hooks.json` at session start — no path substitution required on your end. Every tool call is observed from the moment the plugin is installed.
