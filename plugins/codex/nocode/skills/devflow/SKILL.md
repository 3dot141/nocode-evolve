---
name: devflow
description: "工程任务流程领航（8 阶段 · 4 场景路由）。可被 model 主动调起，也可用户 /调 进入。给\"当前阶段判断 + 下一步建议 + 备选\"，用户拍板，不替用户执行。agent 视角：复杂…"
---
...
- 没有 → 建议用户先走产品流（`nocode:pdflow`），用 request_user_input 三选：
...
3. **末阶段 handoff 标记**：最后一个阶段 task（交接/收口阶段，如 Land）建 update_plan 时带 `metadata: {handoff: true}`——供防跳步 Hook B 识别交接 task

示例：
```
update_plan(subject: "阶段 8: Land",
           description: "调用: nocode:dev-land / Gate: PR merged + 任务流转 + worktree 清理
Sub-steps: ⓪ $dev-land → 1.意图推定 → 2.全景计划 → 3.全自动执行(含 post-merge)")
```
...
2. **加载 skill（硬 Gate）**：update_plan 标 in_progress 后的**第一个动作**必须是 `$<阶段 skill>`。没看到 Skill 调用回执，不许执行任何 sub-step。
...
5. **update_plan completed + 停下报告**：标 completed 后**停下**，向用户报告本阶段完成情况 + 下一步建议（格式见 Step 5）。**不自动进入下一阶段。**
...
❌ 进入 Build 阶段，看到 task description 里已有 "5a.Scope Lock → 5b.Test First → ..."，
   直接照着写代码——没调 $dev-build，丢了 TDD Iron Law 和 slice 循环约束。
```
正确做法：标 in_progress → 先 `$dev-build` 拿到完整指令 → 再按 sub-steps 推进。
...
| 2 | **Env** | Gate Base → `nocode:using-git-worktrees` → Codex workdir | `rule-git-worktree` | worktree 已建并进入（注：Env 不需要独立 nocode skill，逻辑完全由 superpowers skill + rule-git-worktree 覆盖） |
...
每个阶段的子步骤序列。Step 3 update_plan 时抄编号链，Step 5 进入时展开决策点。
...
| Sub-step | 做什么 | 决策 |
|---|---|---|
| 1a. 场景分类 | request_user_input 四选 | Full/Standard/Fix/Mini（拿不准偏 Full） |
...
| 1f. 用户确认 | request_user_input 三选 | 确认/修改/重来（"随你"不算确认） |
...
| 2d. Codex workdir | 切 cwd 到 worktree | — |
...
| 4g. 用户确认 | request_user_input | 确认计划 |
...
- **不跳过 update_plan** — 进入 devflow 必须建 todo
