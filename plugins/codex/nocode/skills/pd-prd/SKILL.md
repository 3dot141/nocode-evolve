...
Task 8: 用户最终确认
  Sub-steps: request_user_input 三选
  Gate: 用户显式确认
...
Task 10: 硬交接 — 调用下一步 skill
  Sub-steps: 报告 PRD 完成 → 按需求形态建议：涉及界面 → 调 $pd-ix（之后可续 pd-vd）；纯后端 / 无界面 → 进 devflow 开发流 → 等用户拍板
...
用 request_user_input 逐字段确认（不一次问完，一次一个）：
...
格式与 ID 约定见 `${PLUGIN_ROOT}/shared/references/path-conventions.md`，开始前 Read。
...
**路径与 ID 格式**：见 `${PLUGIN_ROOT}/shared/references/path-conventions.md`（Step 3 已 Read）。
...
用户确认前做 PRD 评审：Read `references/prd-review.md`（PRD 8 维度）拿维度，然后 `$reviewing`，声明：**对象** = PRD；**领域维度** = PRD 8 维度；**方法** = checklist（或让引擎按对象自选）。引擎产 findings + verdict——流程 / 执行者 / 档位 / 升档 / 降级 / 分级全由引擎承载，本节不复述。Critical 必须修复再让用户确认。
...
- [ ] 用户显式确认（request_user_input 选了"确认"）
- [ ] 文件已保存到正确路径
