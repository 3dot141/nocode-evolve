...
**Spec Self-Review:**
After writing the spec document, run a **light-depth self-review** over it。调 `$reviewing`，声明：**对象** = 刚写的 spec；**方法** = self-review（轻档就地，不派 subagent / codex）；**领域维度** = 下面 4 项 spec 自查维度；**档位** = 轻档。引擎按 self-review 形态就地过维度、就地修复，跳过独立交叉；仅"记录但本轮不修"的遗留才落 findings（`kind = self-audit`）。分级 / schema / 纪律全由引擎承载，本 skill 不复述。
