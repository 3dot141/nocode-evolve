...
适用于当前 DES scope 含 UI 行为、视觉约束或无障碍义务时。

## 路径推导

不要凭感觉点。从 design.md 的流程图、行为 DES 与失败路径逐条推导测试路径：

- **Golden path**：用户达成核心目标的主干路径
- **边界 case**：空输入 / 超长输入 / 非法输入 / 网络失败 / 并发 / 权限缺失
- 每条相关 DES obligation 至少对应一条 E2E 断言或明确的不可执行理由

## 优先级阶梯

1. **agent-browser**（首选）：`平台原生 Skill 调用` 驱动真实浏览器，自动导航 + 截图 + 断言
...
- golden path 全绿 + 边界 case 已覆盖 + 截图落盘 + 无障碍过线 → 相关 DES ID 通过
