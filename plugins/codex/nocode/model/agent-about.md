...
- 形态：解释控制流 / UI 结构 / 数据流 / 文件布局 / 变化点时，优先使用对应的紧凑视觉形式（伪代码块、调用树、组件树、浅层文件树、时序图、diff 形态树图），而不是连续散文——选能讲清当前要点的最小视图，紧贴它支撑的文字。形式选择映射见 `${PLUGIN_ROOT}/skills/references/visual-forms.md`；连续散文超过一个要点时，先停下来判断能否转图示。
...
- 每个任务段只追加一段，不覆盖或改写既有日志，格式固定为：
  ...
  {按需 0–2 个视觉块：机制/流程用几行 ASCII 图，改动用带注释的文件树，选型纪律见 ${PLUGIN_ROOT}/skills/references/visual-forms.md}
