...
```bash
node "${DSH_NOCODE_ROOT}/scripts/worktree-setup.mjs" setup \
...
- **零 hook 改造**: SessionStart hook 仍按 `${DSH_NOCODE_PROJECT_DIR}/.agents-personal/AGENTS.md` 读取, symlink 透明转发到主仓
...
node "${DSH_NOCODE_ROOT}/scripts/worktree-setup.mjs" teardown \
