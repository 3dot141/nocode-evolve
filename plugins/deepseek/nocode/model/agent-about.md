...
结构化决策在回合末写出完整问题与 2–3 个互斥选项，等待用户下一条消息；评估类提问调用 `/red-blue-deep`。
...
- 形态：解释控制流 / UI 结构 / 数据流 / 文件布局 / 变化点时，优先使用对应的紧凑视觉形式（伪代码块、调用树、组件树、浅层文件树、时序图、diff 形态树图），而不是连续散文——选能讲清当前要点的最小视图，紧贴它支撑的文字。形式选择映射见 `{DSH_NOCODE_SKILL_REF}/visual-forms.md`；连续散文超过一个要点时，先停下来判断能否转图示。
...
- 每个任务段只追加一段，不覆盖或改写既有日志，格式固定为：
  ...
  {按需 0–2 个视觉块：机制/流程用几行 ASCII 图，改动用带注释的文件树，选型纪律见 {DSH_NOCODE_SKILL_REF}/visual-forms.md}
...
| `{DSH_NOCODE_SKILL_REF}` | `${DSH_NOCODE_SKILL_REF}` | 共享领域指南绝对路径（env，SessionStart 写入）|
