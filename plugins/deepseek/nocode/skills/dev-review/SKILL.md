...
计划使用文本里程碑列表（同时最多一个进行中）；独立审查使用 `/reviewing`，Land handoff 使用 `/dev-land`。
...
| 轴 / 检查 | 取什么 | 用来做什么 |
|---|---|---|
| 安全轴 | Read `{DSH_NOCODE_SKILL_REF}/security-guide.md` | OWASP / 注入 / 信任边界 |
| 架构轴 | Read `{DSH_NOCODE_SKILL_REF}/architecture-principles.md` | Deep-Shallow / 依赖方向 / seam |
| 数据库（SQL/schema/migration） | 敏感面——自查基础反模式 + 一句话建议升审 | migration 本身命中敏感面提醒 |
| 性能轴 | Read `{DSH_NOCODE_SKILL_REF}/performance-guide.md` | N+1 / 重渲染 / bundle 反模式 |
| 测试质量 | Read `{DSH_NOCODE_SKILL_REF}/testing-guide.md` | DAMP / 替身偏好序 / 金字塔 |
| UI 代码 | Read `{DSH_NOCODE_SKILL_REF}/frontend-guide.md` | 组件模式 / Avoid AI Aesthetic / WCAG |
...
安全 / 架构轴自查时 Read 上表对应 guide。性能详见 `{DSH_NOCODE_SKILL_REF}/performance-guide.md`。
