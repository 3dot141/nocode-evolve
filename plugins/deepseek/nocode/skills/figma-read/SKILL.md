---
name: figma-read
description: "读取 Figma 设计稿。当用户给 figma.com/design 或 figma.com/file 链接，要求读取设计稿节点、提取设计值（字号/颜色/间距/圆角）、渲染节点截图、解析设计变量、对齐 UI 实现或检查样式差异时使用——走 Figma REST API + $FIGMA_TOKEN，不依赖 figma MCP 或浏览器登录。不负责：分析用户已贴的设计稿截图（无需 API）、Figma 原型预览链接（无 inspect 需求）。"
---
