...
Read this reference when creating, resuming, or advancing `design.log.md`. Read `{DSH_NOCODE_SKILL_REF}/grilling-loop.md` for how to choose and ask the next question.
...
Follow `{DSH_NOCODE_SKILL_REF}/grilling-loop.md`. Type `questions.md` files are coverage checks after a half is empty. They do not generate the next question.
