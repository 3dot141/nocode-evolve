...
从 `用户本次调用参数` 解析一个原型目录和可选的 `--interactions <json-path>`。先把两个路径解析为绝对路径并确认存在；不接受其它选项。然后用独立、带引号的 argv 调用验证器：

```bash
node "${DSH_NOCODE_ROOT}/scripts/prototype-verify.mjs" "<absolute-prototype-dir>" --interactions "<absolute-interactions-json>"
```
