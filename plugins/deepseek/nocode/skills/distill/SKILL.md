...
`wiki/index.md` 是控制文件，普通 Read 不计使用次数；一旦需要核对 `pages/` 或 `draft/` 的候选正文，必须调用 `node "${DSH_NOCODE_ROOT}/scripts/wiki-read.mjs" --project-root "$PWD" --path "<wiki-page-path>" --session-id "<current-session-id>"`，并使用返回 receipt 的 content。
