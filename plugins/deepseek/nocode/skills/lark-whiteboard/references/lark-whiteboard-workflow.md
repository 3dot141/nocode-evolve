...
> 需要在文档中批量创建多个画板时，handoff 到 `lark-doc` Skill，由它负责创建、调度并返回各画板的 `board_token`。
...
| 文档 URL 或 doc_id，需要新建画板 | 调用 `lark-doc` Skill 在目标文档中创建空白画板，使用它返回的 `data.new_blocks[0].block_token`（`block_type == "whiteboard"`） |
