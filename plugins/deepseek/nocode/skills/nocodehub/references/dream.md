...
```bash
node "${DSH_NOCODE_ROOT}/scripts/plugin-dream-baseline.mjs" "${DSH_NOCODE_ROOT}"
```
...
| 引用路径有效 | `{DSH_NOCODE_SKILL_REF}`/references/rule 文件引用不悬空 |
...
```bash
node "${DSH_NOCODE_ROOT}/scripts/plugin-dream-baseline.mjs" --set "${DSH_NOCODE_ROOT}"
```
