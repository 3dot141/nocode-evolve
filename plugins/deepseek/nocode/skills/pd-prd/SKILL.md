...
计划使用文本里程碑列表（同时最多一个进行中），决策在回合末写出完整问题与 2–3 个互斥选项；handoff 使用 `/pd-ix`，评审使用 `/reviewing`。
...
格式与 ID 约定见 `{DSH_NOCODE_SKILL_REF}/path-conventions.md`，开始前 Read。
...
**路径与 ID 格式**：见 `{DSH_NOCODE_SKILL_REF}/path-conventions.md`（Step 3 已 Read）。
