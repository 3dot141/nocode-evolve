...
**这个示例的关键点**：不是沿一条线机械下钻,而是每轮从多路径 + 架构关系图挑最承重的耦合缺口;每轮比较 2-3 个完整切片并给推荐度/原因/证据/改选条件;agent 暂定后传播到所有受影响路径,至少三轮且满足闭环条件才终审。已有决策（无 MQ）可以直接淘汰候选,测试目标逐条挂到 Success Criteria。
