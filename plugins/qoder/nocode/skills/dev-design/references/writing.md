...
For forms beyond these four — runtime call trees, UI component trees with state and module boundaries, and diff-shaped views — use the `show-me` skill:

Use `Skill(nocode:show-me)`.

Two standing rules from it apply here:
...
- Pick the smallest view that carries the point; see the `show-me` skill (invocation block above).
