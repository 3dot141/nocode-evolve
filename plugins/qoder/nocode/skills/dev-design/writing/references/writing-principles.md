...
- **approved 的原子收口**：Review verdict、frontmatter `status: approved`、`## 实施设计项清单` 和单一 `docPath` 完整性必须同时成立。详细字段与四态只从 `{QODER_PLUGIN_ROOT}/skills/references/design-traceability.md` 读取，不在写作原则里复制 schema。
