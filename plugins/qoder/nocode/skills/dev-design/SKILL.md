...
方案选择归 `decision/SKILL.md`，详细设计与唯一评审归 `writing/SKILL.md`，可选渲染归 `{QODER_PLUGIN_ROOT}/skills/references/doc-render.md`。跨 Design / Plan / Build / Verify 的基线契约统一 Read `{QODER_PLUGIN_ROOT}/skills/references/design-traceability.md`。
...
Writing 通过 `render-choice` checkpoint 收到用户选择后，协调器 Read `{QODER_PLUGIN_ROOT}/skills/references/doc-render.md` 并按其 handoff 协议执行。协调器不复制 Open Design 的 provider 命令，也不硬编码具体工具面。
