...
- 命中**标题与当前工作项相同**的候选 → 直接填，无需询问。
- 无标题相同项 → 列出搜索候选让用户选。**不要自行从 git 提交 / 工作日志 / 因果链挖掘「语义更正确」的源头工作项**——过度工程且不符合团队惯例（实测：挖掘得到的源头 bug 被用户按推荐列表改选标题相同项替代）。
