...
**4.4 来源类关联字段的候选选择（缺陷来源于需求 / 产研任务 / 缺陷 / 子任务）**

这类 `workitem_related_multi_select` 来源字段在 Meego 页面流转弹窗里是搜索式单选框，带「你可能想选」推荐列表。**服务端必填校验是四选一**（四个来源字段填一个非空即过）。团队实际操作惯例：**直接选与当前工作项标题相同的候选**（本单通常排在推荐首位，如「缺陷来源于缺陷」选本单自身）。

字段优先序按工作性质定：**bug 修复优先「缺陷来源于缺陷」（自引用，本单 ID 写入即过）；feat 优先「缺陷来源于需求」**（MQL 搜同名需求）。首选匹配不到 → 依序试其余三个来源字段 → 四个全落不了值才问用户。

> 🚨 **类型不匹配的写入会静默失败**：`workitem update` 返回空成功但回读 value 为 null（实测 PR #2065：给「来源于需求」填缺陷 ID 被拒收丢弃；改填「来源于缺陷」= 本单自引用即生效过校验）。**每次写入后必须回读确认生效，再 transition-state。**

CLI 拿不到页面推荐列表，按以下**搜索顺序**找标题相同候选（顺序错了会漏检自引用、多走一次人工询问）：

1. **先比对当前工作项自身**：标题相同的候选通常就是本单（自引用惯例），零查询成本；但仅类型匹配的字段接受（见上静默失败教训）。
2. **再按字段对应类型 MQL 搜索**：「缺陷来源于缺陷」先搜缺陷类型；命中即填：

```bash
meegle workitem query --project-key 空间key --mql "SELECT \`工作项id\`, \`缺陷名称\` FROM \`空间key\`.\`缺陷\` WHERE \`缺陷名称\` LIKE '%<标题特征词>%' ORDER BY \`工作项id\` DESC LIMIT 10"
```

3. **仍无命中才搜其余类型**（需求 / 产研任务 / 子任务）；全部无命中 → 列出搜索候选让用户选。**不要自行从 git 提交 / 工作日志 / 因果链挖掘「语义更正确」的源头工作项**——过度工程且不符合团队惯例（实测：挖掘得到的源头 bug 被用户按推荐列表改选标题相同项替代）。

> 🚨 实测教训（260831 fx-data-agents 流转「创建者验收」）：agent 跳过自身比对、只 MQL 搜了「需求」类型，无命中就停下问用户——而页面推荐列表首位正是本单自引用，多走一轮人工。来源四选一字段**先搜缺陷类型 + 含自身**是默认动作，不是兜底。
