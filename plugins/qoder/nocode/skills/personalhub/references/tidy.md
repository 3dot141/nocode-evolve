...
```bash
node "${QODER_PLUGIN_ROOT}/scripts/personal-snapshot.mjs" --json
```
...
```bash
node -e "
import('${QODER_PLUGIN_ROOT}/scripts/personal-snapshot.mjs').then(async ({ resolvePersonalDir }) => {
  const { diffSinceBaseline } = await import('${QODER_PLUGIN_ROOT}/scripts/dream-baseline.mjs');
  const { snapshot } = await import('${QODER_PLUGIN_ROOT}/scripts/personal-snapshot.mjs');
  const personalDir = resolvePersonalDir(process.env.QODER_PROJECT_DIR || process.cwd());
...
对候选范围每页：
1. 用 `node "${QODER_PLUGIN_ROOT}/scripts/wiki-read.mjs" --project-root "$PWD" --path "<wiki-page-path>" --session-id "<current-session-id>"` 读页面及 frontmatter（created、last_updated、maturity、related、sources）
...
```bash
node -e "
import('${QODER_PLUGIN_ROOT}/scripts/dream-baseline.mjs').then(async ({ advanceBaseline }) => {
  const { resolvePersonalDir } = await import('${QODER_PLUGIN_ROOT}/scripts/personal-snapshot.mjs');
  const personalDir = resolvePersonalDir(process.env.QODER_PROJECT_DIR || process.cwd());
