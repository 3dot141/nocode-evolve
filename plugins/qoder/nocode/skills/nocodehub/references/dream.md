...
```bash
node "${QODER_PLUGIN_ROOT}/scripts/plugin-dream-baseline.mjs" "${QODER_PLUGIN_ROOT}"
```
...
| 引用路径有效 | `{QODER_PLUGIN_ROOT}/skills/references`/references/rule 文件引用不悬空 |
...
```bash
node "${QODER_PLUGIN_ROOT}/scripts/plugin-dream-baseline.mjs" --set "${QODER_PLUGIN_ROOT}"
```
