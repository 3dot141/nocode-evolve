...
对每个候选提取元数据。`wiki/pages/` 与 `wiki/draft/` 候选用 `node "${QODER_PLUGIN_ROOT}/scripts/wiki-read.mjs" --project-root "$PWD" --path "<wiki-page-path>" --session-id "<current-session-id>"` 读取并计数；rules/ 和 AGENTS.md 继续普通 Read：
