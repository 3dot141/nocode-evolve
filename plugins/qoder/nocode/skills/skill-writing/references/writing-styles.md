...
- **引用路径**：自己的 references/ 用相对路径（`references/xxx.md`），不写 `{QODER_PLUGIN_ROOT}/skills/<skill-name>/references/xxx.md`——相对路径在 skill 目录搬家时零改动。归属他 skill 领域的材料（如 `reviewing`）→ `平台原生 Skill 调用` 调用，即使知道确切路径也不直指。无单一归属的材料才放共享 `skills/references/`，用 `{QODER_PLUGIN_ROOT}/skills/references/xxx.md` 直引。
