...
Use for README top-level panoramas of directories with a real dataflow/pipeline/state machine; a directory without one should use a boundary diagram instead, not a fake stacked-box pipeline (README writing rules: `{NOCODE_SKILL_REF}/readme-writing.md`).
