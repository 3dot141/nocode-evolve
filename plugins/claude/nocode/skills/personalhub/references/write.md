...
对每个 wiki 候选，先普通 Read 控制文件 `wiki/index.md`；需要核对已有 `pages/`/`draft/` 正文时用 `node "${CLAUDE_PLUGIN_ROOT}/scripts/wiki-read.mjs" --project-root "$PWD" --path "<wiki-page-path>" --session-id "<current-session-id>"`，再判与已有页关系：
