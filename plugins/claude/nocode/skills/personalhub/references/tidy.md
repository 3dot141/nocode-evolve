...
对候选范围每页：
1. 用 `node "${CLAUDE_PLUGIN_ROOT}/scripts/wiki-read.mjs" --project-root "$PWD" --path "<wiki-page-path>" --session-id "<current-session-id>"` 读页面及 frontmatter（created、last_updated、maturity、related、sources）
