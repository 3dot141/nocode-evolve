...
对每个候选提取元数据。wiki `pages/`/`draft/` 候选必须用 `Capability(personal-knowledge.page.read, {"sessionId":"<current-session-id>","path":"<wiki-page-path>"})` 读取（成功后自动计数）；vault 候选继续用普通 Read：
