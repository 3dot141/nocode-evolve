...
<dir-path>
DREAM_EOF
) && node "${CLAUDE_PLUGIN_ROOT}/skills/projecthub/scripts/project-tree-detect.mjs" detect "$DIR_PATH"
```
...
- 用户选 **要** → 调用：
  ...
  <dir-path>
  DREAM_EOF
  ) && node "${CLAUDE_PLUGIN_ROOT}/skills/projecthub/scripts/project-tree-detect.mjs" find-root "$DIR_PATH"
  ```
...
<gitRoot>
DREAM_EOF2
) && node "${CLAUDE_PLUGIN_ROOT}/skills/projecthub/scripts/project-tree-detect.mjs" ref-name "$DIR_PATH" "$GIT_ROOT"
```
...
import('${CLAUDE_PLUGIN_ROOT}/skills/projecthub/scripts/dream-baseline.mjs').then(({ diffSinceBaseline }) => {
...
import('${CLAUDE_PLUGIN_ROOT}/skills/projecthub/scripts/dream-baseline.mjs').then(({ advanceBaseline, projectLockDir }) => {
  const gitRoot = process.env.DREAM_GIT_ROOT;
  const refName = process.env.DREAM_REF_NAME;
