...
创建 worktree 使用 `git worktree add "<absolute-worktree-path>" -b "<BRANCH_NAME>" "<optional-base-ref>"`；后续每次文件与命令操作都使用该绝对路径，不模拟会话级目录切换。
...
```bash
node "${NOCODE_PLUGIN_ROOT}/scripts/worktree-setup.mjs" setup \
...
- **零 hook 改造**: SessionStart hook 仍按 `${NOCODE_PROJECT_DIR}/.agents-personal/AGENTS.md` 读取, symlink 透明转发到主仓
...
node "${NOCODE_PLUGIN_ROOT}/scripts/worktree-setup.mjs" teardown \
