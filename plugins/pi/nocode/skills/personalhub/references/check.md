# /personalhub check：.agents-personal/ 健康检查

检查当前项目的 `.agents-personal/` 是否健康——wiki 结构完整、rules 引用正确、AGENTS.md 变量和插件当前定义对齐。

被 `/personalhub write`（写入前检查）和 `/personalhub tidy`（维护扫描）调用，也可独立跑。
...
确认 `.agents-personal/` 存在。不存在 → 报 "未初始化，跑 `/personalhub init`" 并停。
...
| # | 检查 | 严重度 | 做法 |
|---|---|---|---|
| V1 | 变量名不在插件当前列表 | warn | 调 `node "${NOCODE_PLUGIN_ROOT}/scripts/personal-lint.mjs" --json`，取 staleVars |
