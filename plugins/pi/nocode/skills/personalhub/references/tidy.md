...
# /personalhub tidy：.agents-personal/ 自主维护
...
```bash
node "${NOCODE_PLUGIN_ROOT}/scripts/personal-snapshot.mjs" --json
```
...
```bash
node -e "
import('${NOCODE_PLUGIN_ROOT}/scripts/personal-snapshot.mjs').then(async ({ resolvePersonalDir }) => {
  const { diffSinceBaseline } = await import('${NOCODE_PLUGIN_ROOT}/scripts/dream-baseline.mjs');
  const { snapshot } = await import('${NOCODE_PLUGIN_ROOT}/scripts/personal-snapshot.mjs');
  const personalDir = resolvePersonalDir(process.env.NOCODE_PROJECT_DIR || process.cwd());
...
| `[]`（有 baseline，diff 为空） | **秒回**：直接输出 `wiki 状态良好，无需维护动作。`，命令结束，不进入 Phase 2/3，也不调 `personalhub check` |
| 非空数组 | 只对 `changedFiles` 里列出的 wiki 页做深度检查；额外执行下方**跨域.3 — related 路径变化检测**；结果与 `changedFiles` 覆盖的页面合并成本轮候选范围 |

`changedFiles` 非 `null` 时才完整读取并执行 `references/check.md` 获取健康状态，然后对**候选范围**（而不是 `wiki/draft/` + `wiki/pages/` 全部）做深度检查：

对候选范围每页：
1. 用 `node "${NOCODE_PLUGIN_ROOT}/scripts/wiki-read.mjs" --project-root "$PWD" --path "<wiki-page-path>" --session-id "<current-session-id>"` 读页面及 frontmatter（created、last_updated、maturity、related、sources）
...
| **promote** | draft/ 已丰富（2+ sources、有交叉引用）但未提升到 pages/ | 完整读取并执行 `references/write.md` 中的 promote 流程 |
...
0 候选时："wiki 状态良好，无需维护动作。" + 附 personalhub check 报告。
...
- **merge**：通过 `wiki-read.mjs` 分别读取两页并记录引用 → 融合内容到保留页 → 删除被合并页（走删除护栏） → 更新引用
- **promote**：完整读取并执行 `references/write.md` 中的 promote 流程（draft → pages，删除 draft 原文件）
...
```bash
node -e "
import('${NOCODE_PLUGIN_ROOT}/scripts/dream-baseline.mjs').then(async ({ advanceBaseline }) => {
  const { resolvePersonalDir } = await import('${NOCODE_PLUGIN_ROOT}/scripts/personal-snapshot.mjs');
  const personalDir = resolvePersonalDir(process.env.NOCODE_PROJECT_DIR || process.cwd());
...
| 命令 | dream 怎么用它 |
|---|---|
| personalhub check | Phase 1 开头读取 `references/check.md`，获取健康基线 |
| personalhub write | promote 动作读取 `references/write.md` 执行 |
| personalhub search | 不调用 |
| personalhub snap | Phase 1 Step 0 调用（`--json` 触发 ensureNestedRepo/迁移 + 作为 `diffSinceBaseline` 的 `prepareFn` 落定 working tree），SessionStart 也仍会自动跑 |
