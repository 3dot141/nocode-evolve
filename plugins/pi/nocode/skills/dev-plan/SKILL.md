...
进入 Full Plan 时 Read `{NOCODE_PLUGIN_ROOT}/skills/references/design-traceability.md`。Design Registry 是 task `designCovers` 与反向 orphan 检查的唯一协议来源；不得在 Plan 内另造一套状态或 schema。
...
每个 task 用 `references/task-template.md` 格式。路径/约束 ID 约定见 `{NOCODE_PLUGIN_ROOT}/skills/references/path-conventions.md`。
...
| 场景 | Read | 用来做什么 |
|---|---|---|
| 涉及文件结构/模块边界 | `{NOCODE_PLUGIN_ROOT}/skills/references/architecture-principles.md` | Deep Module / 依赖分类 / seam 纪律，指导怎么拆文件、定接口 |
| 碰用户输入/认证/数据 | `{NOCODE_PLUGIN_ROOT}/skills/references/security-guide.md` | 威胁模型 / OWASP 防护模式，决定这段代码该用什么防注入写法 |
| 碰数据库查询/前端渲染 | `{NOCODE_PLUGIN_ROOT}/skills/references/performance-guide.md` | N+1 / 缓存 / 懒加载模式选型 |
| 碰 UI 组件 | `{NOCODE_PLUGIN_ROOT}/skills/references/frontend-guide.md` | 组件模式 / 设计系统遵循 |
| 写测试代码前 | `{NOCODE_PLUGIN_ROOT}/skills/references/testing-guide.md` | 测试替身怎么选 / 测试金字塔怎么分层 / DAMP 原则 |
...
**UI task 的验证方式**：涉及 UI 样式且存在设计基线（样张 / 原型截图 / 设计稿）的 task，Step 8d 声明的验证方式写「设计值对齐 + 截图对比基线」（方法与词表见 `{NOCODE_PLUGIN_ROOT}/skills/references/frontend-guide.md`「设计基线对齐」节），不硬套测试命令；无基线则标注跳过。
