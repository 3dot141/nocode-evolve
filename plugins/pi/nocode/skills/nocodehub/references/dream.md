...
# /nocodehub dream：插件仓库自维护巡检

检查 nocode 插件仓库自身（rules/skills/commands）与预期状态的偏差，提议修复动作，用户确认后执行。独立于 `/nocodehub write`——distill 管写入，dream 管巡检维护。
...
```bash
node "${NOCODE_PLUGIN_ROOT}/scripts/plugin-dream-baseline.mjs" "${NOCODE_PLUGIN_ROOT}"
```
...
| 非 `null` 且 `commitDiff`/`dirtyFiles` 均为空 | `false` | **秒回**，不进入 Phase 1，直接输出：`✓ nocodehub dream：自上次检查以来业务源码、adapter、packager、plugin/metadata.json 与 marketplace 无变化，无需维护`，命令结束 |
...
| 引用路径有效 | `{NOCODE_PLUGIN_ROOT}/skills/references`/references/rule 文件引用不悬空 |
...
| # | 对象 | 检测 | 动作 | 理由 |
|---|---|---|---|---|
| 1 | plugin rule `foo` | R2 触发质量 | 建议优化(委托 `references/write.md`) | description 只写"需要时读"，无法自判命中 |
...
- **建议式**（Layer2 全部）：不自动改——rule 类完整读取并执行 `references/write.md`，skill 类调用 `skill-writing` Skill；两者都传入当前 request、stage、restate、artifacts、constraints、planRef 和 decision，由用户在委托流程里最终拍板
...
```bash
node "${NOCODE_PLUGIN_ROOT}/scripts/plugin-dream-baseline.mjs" --set "${NOCODE_PLUGIN_ROOT}"
```

`setBaseline` 把 baseline 推进到当前 HEAD——这样下次 `/nocodehub dream` 才能拿"这次处理完的点"作为新起点做增量判断；只有 Phase 0 判定"秒回"（无任何变化）时才跳过本步骤，因为那种情况根本不会执行到 Phase 1-3。
...
nocodehub dream 完成：
  ...
  → 委托: plugin rule `foo` description 优化，完整读取并执行 `references/write.md`
...
SessionStart 已自动跑 rule 与 Hook generator 的一致性检查，但只 warn 不阻断、不修复。`/nocodehub dream` 是开发者主动触发的交互式扫描 + 提议修复 + 可执行，两者互补不重复。
