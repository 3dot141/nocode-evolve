...
进入 Build 时 Read `{NOCODE_PLUGIN_ROOT}/skills/references/design-traceability.md` 与 `{NOCODE_PLUGIN_ROOT}/skills/references/source-comment-contract.md`。Plan 的 `designRevision` / `designDigest` 是不可变基线，`designCovers` 是已确认 task scope 的一部分：执行方只能实现并回报，不能自行更换基线或 Design ID。
...
- **When** task 涉及 UI 样式且存在设计基线（样张 / 原型截图 / 设计稿）→ 实现循环加入设计值对齐（Read `{NOCODE_PLUGIN_ROOT}/skills/references/frontend-guide.md`「设计基线对齐」节）：实现 → devtools 对比设计值 → 调整 → 复检，作为样式代码的红绿等价物；对齐记录留在 task 产出里供 Verify 抽查。无基线则跳过并标注
