...
4. **设计基线** — Plan 与 Design 的 `designRevision` / `designDigest`
5. **Plan 全局视图** — 依赖图 + 全部 task 列表（让 reviewer 知道本 task 在整体中的位置）
6. **前置 task 产出摘要** — 本 task 依赖的 task 改了哪些文件、暴露了哪些接口（跨 task 一致性）
...
Design baseline: designRevision={DESIGN_REVISION}, designDigest={DESIGN_DIGEST}.
If the current approved Design differs, report a stale-baseline issue and stop the compliance judgment.
