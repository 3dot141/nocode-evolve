...
> IX↔VD 分工判据与共享术语（状态覆盖 / 行为规格 / 控件四态、三层命名 tokens/components/patterns）单源在 `{NOCODE_PLUGIN_ROOT}/skills/references/ix-vd-contract.md`——`.ix.md` 定义的行为语义不得改动；原型需要 IX 未定义的行为 → 回流登记，不就地定。
