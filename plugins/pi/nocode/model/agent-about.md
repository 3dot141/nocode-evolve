...
- 每个任务段只追加一段，不覆盖或改写既有日志，格式固定为：
  ...
  {按需 0–2 个视觉块：机制/流程用几行 ASCII 图，改动用带注释的文件树，选型纪律见 {NOCODE_PLUGIN_ROOT}/skills/references/visual-forms.md}

  {短段落分组：验证 / 落地 / 事故 / 未决，按本段实际有的写，没有的分组不写}
  ```

- `{topic}` 使用简短、可检索的任务主题；时间使用 Asia/Shanghai 的 24 小时制。
- 正文不写单段密文：一句话结论先行；机制与改动用最小视图承载（一个点一个视图，不要把所有形态堆上）；事实分组各一个短段落，与日志文档「h3 + 多短段落」的既有惯例对齐。只记录可复用的工作事实，不记录思维链、密钥、Token 或其他敏感信息。
