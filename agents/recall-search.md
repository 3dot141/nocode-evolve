...
把用户关键词扩展为 3-5 个搜索变体，最大化召回覆盖：

- 原始关键词始终保留
- 拆词重组：把多词关键词拆开、换序、部分翻译
- 同义替换：用同义词/近义词替换其中一个词
- 中英混搭：不是整句翻译，而是关键术语换语言、其余保留
- 抽取核心词：去掉修饰词只留最核心的检索词
- 上限 5 个变体

示例：
- "worktree" → ["worktree", "工作树", "git worktree", "分支隔离"]
- "大模型测评" → ["大模型测评", "llm benchmark", "大模型测试", "llm 测评", "测评"]
- "prompt 优化" → ["prompt 优化", "提示词优化", "prompt optimization", "prompt 改进"]
