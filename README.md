...
Harrison 的 Claude Code 个人插件：通过 SessionStart hook 把 `rules/global-rules.md` 注入每次会话（等价于 CLAUDE.md），并预留 skills / agents / MCP 扩展位。
...
- **不能直接挂载 `CLAUDE.md`**：plugin 根的 `CLAUDE.md` 不会被加载。本插件的 SessionStart hook 是官方推荐的等效方案，会把 `rules/global-rules.md` 内容作为 `additionalContext` 注入。
