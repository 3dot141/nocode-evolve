...
把交互结构 + 视觉方向 + 冻结的设计系统（tokens + components，Step 3 样张已拍板）拼成可看可走的原型。两条交付线（Claude Design / 本地 HTML）步骤相同、实现不同。**先回查 Step 2b 选定的交付方式，别凭记忆判断走哪条线。**
...
**100% 覆盖（三档相同）**：IA 中的每个页面/视图都必须在原型中实现——独立页面有自己的文件，嵌入组件在宿主页面内实现。不存在"设计覆盖但原型未实现"的中间态，**也不存在"低保真先出关键页"**——覆盖度不随保真度打折。

| | Claude Design 线 | 本地 HTML 线 |
|---|---|---|
| 怎么出 | `Skill(nocode:claude-design)` → `claude-design <brief>` | 本地写多个 `.html` 文件 |
| 喂什么 | brief = IA + 交互清单 + 场景脚本 + 视觉方向 + 保真度；挂 template + Step 3 设计系统 | IA + 交互清单 + 场景脚本 + 视觉方向 + 冻结 tokens/components/样张 |
| 低保真（拆分） | 每个独立页面一个文件（含宿主内嵌入组件），静态 | 每个独立页面一个文件（含宿主内嵌入组件），静态 |
| 高保真（组合） | 保留独立页面 + 额外一个组合文件（融合全部页面代码，JS tab 切换/弹窗） | 多文件之间用 URL 跳转，每个文件内做弹窗 |
...
1. **设计系统引用** — "用 \<设计系统名\> 设计系统"（Step 3 创建/复用并经样张拍板的）
2. **页面结构（IA）** — 从 `.ix.md` 的 IA 抄：每个页面有哪些区块，**全部页面，无例外**
3. **交互清单 + 行为规格要点** — 每个页面用户能做什么操作、关键行为语义（浮层维持/退出）
4. **场景脚本** — `.ix.md` 的走查实例，作为 demo 编排依据
5. **视觉方向 + 保真度** — Step 2c 方向（一句话 + 参考产品）+ 明确档位与「此档不包含」

### 挂 template 和 design system

- **挂 design system**：brief 里写"用 \<名\> 设计系统"，Claude Design 自动引用该项目的 tokens + components 渲染
...
| 交互 | 不强调交互逻辑 | 组合文件内：tab 导航 + 弹窗 + 边界态（按行为规格） |
| 屏数 | **IA 全部页面/视图**（覆盖度不打折） | **IA 全部页面/视图** + 完整流程可走 |
...
- 点"+导入" → 弹出模态对话框 → 确认后关闭
- 列表区：用按钮切换 empty / loading / 正常 / error 边界态
...
**前提**：设计系统已冻结（Step 3 样张拍板）、视觉方向已定（Step 2c）。
...
- 设计系统引用："用 <设计系统名> 设计系统"（含冻结 tokens 变量清单 + 样张路径）
- 视觉方向：Step 2c 确定的方向（一句话 + 参考产品）
- 保真度：低保真 / 高保真 / 完整实现（+「此档不包含」）
- 导航结构（全站统一）
...
- 融合代码到 `prototype.html`：顶部 tab 切换 + JS 弹窗/抽屉 + 边界态
...
- 所有视觉值引用冻结 tokens（`<link>` 引入共享 `tokens.css` 或等价物），禁硬编码、禁改名
...
1. **控件四态落地** — 组件级样式已在 Step 3 定义，页面继承；逐控件确认无遗漏（Playwright Phase 3 逐控件验证）
2. **边界态全覆盖** — 列表/数据区覆盖 empty / loading / error，不只画正常态。用 `data-state` 属性切换
3. **跨页导航链路** — 按 `.ix.md` 场景脚本，从任一入口走完一条完整使用路径再回来，链路无断点
...
- **渐进式** — 在 Step 2d 确认的升级基线上加东西，不推翻重来。低保真→高保真 = 加交互；高保真→完整实现 = 加控件四态 + 边界态 + 链路
...
| 资源详情 | 嵌入组件 | library.html（Drawer） | 行点击 | data-state 按钮 |
```

## 检查点

原型产出后验证：
- 保真度对应的完整度达标（低保真有视觉渲染；高保真可交互；完整实现有控件四态 + 边界态 + 链路）
- IA 的页面/视图 100% 覆盖：独立页面有文件，嵌入组件在宿主页面内实现，无遗漏（三档同）
