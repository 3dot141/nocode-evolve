...
2. **独立路 = 隔离上下文执行**：默认单路 Codex（经 `rule-codex-review` 单一通道，`平台原生 agent/plan/decision 工具` 包 Bash 执行）；调用报错才 fallback 改派 general-purpose subagent 单跑（标「同模型（降级）」），非并行双跑。传给独立路的内容 = 打包任务原文（评审对象 + 全部场景清单）+ **Context Capsule**（skeleton §4.1：剥结论、留事实）——不传主路 findings / 倾向。
