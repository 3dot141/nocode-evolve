...
| 反模式 | 正确做法 |
|---|---|
| "先写框架，代码执行时再填" | 写不出真实代码 = 没想清楚，回 Step 1 |
| `...` / `TODO` / "类似这样" | 占位符正是藏判断题的地方 |
| 按层横切（"先建所有 model"） | 垂直切片——每片做完都可验证 |
| 简单的先做，难的留后面 | risk-first：不确定性留到投入最大时才暴露 |
| 某 task > 5 文件不拆 | L 任务把多个设计决策压成一句话 |
