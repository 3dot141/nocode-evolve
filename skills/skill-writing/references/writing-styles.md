...
Workflow skill 的 SKILL.md 必须包含：**Step 0 TaskCreate**（所有任务开局建齐，各带 Sub-steps + Gate）、**每步 Enter/Exit Gate**、**Global Exit Gate**。Gate 必须客观可验（yes/no、pass/fail、阈值）——不用主观词。
...
- **引用路径**：自己的 references/ 用相对路径（`references/xxx.md`），不写 `{CLAUDE_PLUGIN_ROOT}/skills/<skill-name>/references/xxx.md`——相对路径在 skill 目录搬家时零改动。归属他 skill 领域的材料（如 `reviewing`）→ `Skill(nocode:<name>)` 调用，即使知道确切路径也不直指。无单一归属的材料才放共享 `skills/references/`，用 `{NOCODE_SKILL_REF}/xxx.md` 直引。
...
**Layer 1 · 结构化评审走 reviewing 引擎**：若是多维评审（多评审维度 + 分档 + findings 分级 + 收尾），引导它调 `Skill(nocode:reviewing)` 而不是在新 skill 里重造评审流程——该步骤写成：调引擎，传评审对象 + 本 skill 自己的领域评审维度（inline 或指向新 skill 自己的 `references/xxx-review.md`）+ 可选方法。引擎自闭环处理分档、方法选择、升档、findings/verdict 格式——新 skill 永不直读 reviewing 的内部文件、永不硬编码指向它的路径。

**Layer 2 · 谁跑独立交叉**（轻量单点自查跳过 Layer 1 直接选）：
- **评估/决策**（"这方案靠谱吗？"）→ 指向 `Skill(nocode:red-blue-deep)`
- **产出评审**（"这代码/文档有没有问题？"）→ 先自审，分歧时升级单次 codex 独立评审（经 `Skill(nocode:reviewing)` 达成）
...
问题当场修；修不了的显式记录。自审是底线不是天花板——发现真正的关键缺陷、或对象明显高风险时，升级 `Skill(nocode:red-blue-deep)` 独立评审。
