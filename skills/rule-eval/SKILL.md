---
name: rule-eval
description: 跑某条 route 路由规则的触发率 eval (momentum-aware route-recall + 混淆矩阵 + intent-signal), 出报告
...
测某条 route 路由规则"该触发时被认出"的比例（momentum-aware route-recall），出报告。设计见 `docs/plans/3dot141/260526-rule-trigger-eval-design.md`。

路由定义来源：`skills/route/SKILL.md` 的 `rule-routes` 生成区（marker `<!-- BEGIN/END generated: rule-routes -->` 之间，由 `node hooks/generate.mjs` 从 `rules/manifest.json` 填充）。**若该 marker 区间缺失或为空**（route 未生成），先提示用户跑 `node hooks/generate.mjs` 再重试，不要在空路由表上跑 eval。
...
### 1. 读注入载荷 + fixture
- SessionStart 载荷（重建会话上下文）：`model/agent-about.md`、`model/agent-karpathy.md`、`model/agent-catalog.md`（精简桶路由，及 `model/` 下其他 `*.md`）。
- route 路由表：`skills/route/SKILL.md` 的 `rule-routes` 生成区（marker 之间的完整 rule 路由定义——触发 / 读 / 摘要 / guard）。这是 route-recall 的判定依据，命中粗桶后 agent 才会调 `Skill(nocode-evolve:route)` 看到它。
...
---
以下是你的行为基线与规则路由表（你的常驻注入 + route skill 加载内容）：
<model/agent-about.md 全文>
<model/agent-karpathy.md 全文>
<model/agent-catalog.md 全文>            # 精简桶路由（常驻）
<skills/route/SKILL.md 的 rule-routes 生成区全文>   # 命中粗桶调 Skill(nocode-evolve:route) 拿到的完整路由表
---
用户说："<样本话术>"
...
### 5. `--all`
对 `eval/cases/` 下每条 fixture 跑；汇总**混淆矩阵**（各 rule 正样本的 `primary_route` 落点，对角=命中、非对角=串扰）；列**未覆盖**的 route 路由规则（在 `skills/route/SKILL.md` 的 `rule-routes` 生成区有条目但无 fixture）。
