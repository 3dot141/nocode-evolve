...
- **`bkt pr create` 不要带 `--reviewer` / `--with-default-reviewers`** —— 部分失败会让整个 create 失败、且 cross-fork 时 default-reviewers 解析常出错。nocode-evolve 流程: `bkt pr create` 不带 reviewer → 用 `bkt pr edit <id> --reviewer ...` 后加(允许部分失败 + 大小写 fallback)。详见 `dev-finish-branch` skill Gate Title-Body/PR + `skills/dev-finish-branch/references/pr-flow-bkt-appendix.md` Workflow B。
