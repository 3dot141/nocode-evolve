...
2. **低保真：IA + wireframe** — 信息架构 + 关键流程 + 线框图，**先批准**
...
4. **关键流程数** — "核心要设计几条用户流程？默认从 PRD 的主 User Story 提。"
...
2. **关键流程**：每条核心流程从入口到完成目标的步骤（entry → steps → done），对照 PRD 的 User Story
...
**① PRD 逐条走查**（硬约束）：PRD 的每条 User Story / 核心场景，在设计里都有对应的界面路径吗？逐条点名，缺的补、PRD 没有的删（脱离 PRD 自由发挥要回头确认）。
...
## Key Flows
[每条关键流程: entry → steps → done, 标注对应 PRD User Story]
...
- [ ] 给了 2-3 个视觉方向，用户已选
- [ ] PRD Coverage 逐条核过，无遗漏 User Story
- [ ] 选了中/高保真则原型/静态稿已产出
- [ ] 文件已保存到正确路径
...
- 只给一个视觉方向（没发散）
- 没对照 PRD 逐条核（可能漏了 User Story 对应界面）
