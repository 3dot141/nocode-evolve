...
用户确认前，按 `{NOCODE_SKILL_REF}/prd-review.md` 做 PRD 评审。prd-review 已引入 `reviewing` 框架：Read `{NOCODE_SKILL_REF}/reviewing/skeleton.md` 套通用流程骨架（分档 → 独立交叉 → findings 分级 → 收口）+ Read `{NOCODE_SKILL_REF}/reviewing/findings-contract.md` 套 findings 契约，按 prd-review 的 PRD 8 维度（框架第 3 步）评审。重档走 red-blue 双模型交叉——Claude 做蓝军、Codex 做红军（CLAIM 剥离不传蓝军结论），Critical 必须修复再让用户确认。
