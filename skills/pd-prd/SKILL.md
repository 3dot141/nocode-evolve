...
3. **领域与路径建模** — 识别领域 → 展开使用路径 → 跨域 → 系统 → 约束
4. **核心流程图** — 为每组相关路径画流程图，图中标注路径 ID
5. **起草 PRD** — 按模板写（图驱动结构），标注 [CONFIRMED]/[ASSUMED]/[TBD]
...
路径建模完成后，**先画图再写文档**。图是读者理解系统的入口——看完图建立全局地图，需要精确定义时顺着路径 ID 跳到详述。

1. **系统架构总览图**：一张图展示所有业务领域及其连接关系（mermaid graph / flowchart）
2. **按流程分组画图**：把相关领域的路径组成端到端流程，每张图覆盖一组关联路径。图中每个节点/步骤标注路径 ID（如 `创建.P1`），指向 §5 的详述。分组按业务逻辑聚合，不机械按领域一一对应——一张图可以串多个领域
3. **领域关系总图**：总览所有领域的连接和数据流方向，帮读者看清领域间的依赖关系

**图的原则**：
- 图说"怎么流转"，详述说"每步精确定义"——不重复，互相引用
- 用 mermaid flowchart / sequenceDiagram / graph，不用 ASCII art（mermaid 可渲染可编辑）
...
读者先看图建立全局理解，需要精确定义时顺着路径 ID 跳到 §5。

### 4.1 系统架构总览图
[mermaid graph — 所有领域及其连接]

### 4.{N} {流程名}
> 引用路径: {领域}.P1, {领域}.P2, ...

[mermaid flowchart/sequenceDiagram — 端到端流程]
[图中每个节点标注路径 ID]

### 4.{末} 领域关系总图
[mermaid graph — 总览所有领域的连接和数据流方向]
...
| 图覆盖 | §4 的流程图覆盖了所有领域路径吗？有没有路径在图里完全没出现？ |
| 图↔详述对齐 | 图中标注的路径 ID 在 §5 都有对应详述？详述里的路径在图中都有出现？ |
...
- [ ] PRD 文档已产出，包含所有核心字段（§1-§10）
- [ ] 核心流程图（§4）覆盖所有领域路径，图中路径 ID 与 §5 详述双向对齐
...
- §4 流程图缺失或只有一张总览图（没有按流程分组画细节图）
- 图中节点没标路径 ID（图和详述断连，读者无法跳转）
- §5 详述里有路径在 §4 图中完全没出现（图覆盖不完整）
