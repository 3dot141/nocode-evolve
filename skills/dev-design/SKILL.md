...
2. **列举的阶段内确认**（协调器已知、不隐藏）：select 的 Decision Packet 用户审核（Step 8a——Packet 写文件 → 用户逐条编辑 → 读回应用,每次必过）、refine 的文档结构确认（Step 2 章节大纲 + 结构骨架）、refine 唯一评审的 findings 逐条 fix/skip、render 的"是否渲染"选择。
