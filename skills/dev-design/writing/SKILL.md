...
当设计依赖代码无法表达的顺序不变量、外部契约、失败语义、兼容 / sunset 条件、magic value 来源或 workaround 删除条件时，Read `{NOCODE_SKILL_REF}/source-comment-contract.md`，为需要在源码保留的理由登记 `COMMENT-*`。Registry 记录应保存的知识、来源与变更 / 删除条件，不预写具体代码注释；理由能由命名、类型或结构完整表达时不登记，不按文件数量批量制造 COMMENT 项。
...
- `{NOCODE_SKILL_REF}/design-traceability.md`
- `{NOCODE_SKILL_REF}/doc-render.md`
