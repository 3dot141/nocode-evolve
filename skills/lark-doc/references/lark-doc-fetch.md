...
返回中可能含 `<sheet>`、`<bitable>`、`<cite file-type="sheets|bitable">`。内部数据无法通过 `docs +fetch` 获取，提取 `token` 等属性后切到 `lark-sheets` Skill / `lark-base` Skill 下钻，详见 [SKILL.md 快速决策](../SKILL.md) 路由表。
