---
name: lark-doc
description: "当用户给出飞书/Lark/豆包 docx 或 wiki 文档 URL/token，或需要读取、创建、编辑文档正文、处理文档媒体、封面或思维笔记时使用。不负责评论、表格/Base 数据操作或知识空间层级管理。"
---
...
1. 调用 `lark-shared` Skill — 认证、权限处理、全局参数（所有操作通用）
...
## 快速决策
- 用户要**复制文档 / 创建文档副本 / 另存为副本**时，切到 `lark-drive` Skill，按其中的复制指引使用 `lark-cli drive files copy`；不要用 `docs +fetch` + `docs +create` 重建正文，也不要走 `drive +export` / `drive +import`。
- 先判定任务路径：找文档 / 导入导出走 `lark-drive` Skill；只读 / 摘要用 `docs +fetch` 默认 `simple`；明确旧文本 → 新文本直接 `str_replace`；只有 block 链接、评论锚点、插入 / 替换 / 删除 / 移动才局部 fetch `with-ids`；保真改写已有内容才读 `full`
...
| 标签 / 属性 | 提取字段 | 切到技能 |
|-|-|-|
| `<sheet token="..." sheet-id="...">` | `token` -> spreadsheet_token, `sheet-id` | `lark-sheets` Skill |
| `<bitable token="..." table-id="...">` | `token` -> app_token, `table-id` | `lark-base` Skill |
| `<cite type="doc" file-type="sheets" token="..." sheet-id="...">` | 同 `<sheet>` | `lark-sheets` Skill |
| `<cite type="doc" file-type="bitable" token="..." table-id="...">` | 同 `<bitable>` | `lark-base` Skill |
| `<vc-transcribe-tab vc-node-id="...">` | `vc-node-id` -> note_id | `lark-note` Skill：先 `note +detail --note-id <vc-node-id>` |
...
| `+whiteboard-update` | Alias of `whiteboard +update`. Update an existing whiteboard with DSL, Mermaid or PlantUML. Prefer `whiteboard +update`; hand off to the `lark-whiteboard` Skill for details. |

## 不在本 Skill 范围

- 文档评论管理 → `lark-drive` Skill
- 电子表格或 Base 的数据操作 → `lark-sheets` / `lark-base` Skill
- 云空间文件上传、下载、权限管理 → `lark-drive` Skill
