...
评估/拍板类提问的红蓝军对抗框架。**本 skill 是独立入口**——用户直接问评估/拍板问题（「X 行不行 / 选 A 还是 B / 值得吗」）时直触；同时被 dev-plan（计划两轮审视）、pd-vd、devflow 经 `Skill(nocode-evolve:red-blue-deep)` 调起。

本 skill 的**方法实现已抽到 reviewing 框架的方法 card，card 是单源**。本壳只保留「档位判定入口 + 调用约定」，不复述方法细则（避免双源漂移）。

## 方法实现见 card（单源）

红蓝对抗的完整方法——档位判定表与边界示例、light/heavy 两档、sequential-thinking 硬 gate、第一性原理 → 蓝军 → 独立审查 → 结论四步、CLAIM 剥离、subagent + codex 并行双跑与降级、Doubt Theater 检测、三轮收敛上限、findings/verdict 输出契约——**单源在方法 card**：
...
模糊时按「能否 5 分钟内回滚」判：能 → 轻档，不能 → 重档。**完整判据表 + 边界示例 + 「多方案僵持」定义在 card**，定不准时回 card 对表。

> 调用方可强制档位：dev-plan 两轮审视、pd-vd 都按「跨模块不可逆」要求**强制重档**——尊重调用方的强制档，不擅自降级。
...
- **行为不变**：light/heavy 档位判定、sequential-thinking 硬 gate、codex 并行双跑与降级、第一性原理四步——全部经 card 落地，与吸收进框架前一致。
