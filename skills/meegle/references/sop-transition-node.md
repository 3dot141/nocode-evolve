...
传入 `mode = "unfinished"` 仅查未完成必填项。从返回中识别每个字段的**判别键是 `class`**（`node_field` / `field`，注意不是 `form_item_type`——那是 `workflow get-node` 返回里的键名，两个接口的键不一致，实测 260817）和 `field_type_key`。
...
| `select` / `radio` | option_name 匹配 → `"option_id"` 字符串。节点表单里的 radio 若 `meta-node-fields` 查不到 options（如 `field_90f734` 研发处理后评估），直接传 **option_name 原文**（如 `"已处理"`）经 `workitem update` 落值成功（实测 260817）；传 `system_passed` 之类的猜测 id 会"success 但不落值" |
