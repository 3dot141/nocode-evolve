...
这类 `workitem_related_multi_select` 来源字段在 Meego 页面流转弹窗里是搜索式单选框，带「你可能想选」推荐列表。团队实际操作惯例：**直接选与当前工作项标题相同的候选**（本单通常排在推荐首位，如「缺陷来源于缺陷」选本单自身）。

CLI 拿不到页面推荐列表，按以下**搜索顺序**找标题相同候选（顺序错了会漏检自引用、多走一次人工询问）：

1. **先比对当前工作项自身**：标题相同的候选通常就是本单（自引用惯例），零查询成本。
