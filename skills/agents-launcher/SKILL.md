...
用户请求 (起 dev / 重启 / 停服)
    ...
    ├── 主仓启动 ──────────────────────────────────────────► Step 3 (执行脚本)
    │
    └── worktree 启动 ──► Step 1 (cp) ──► Step 2 (联调对齐) ──► Step 3 (执行脚本)

停服 ─────────────────────────► TaskStop 已知 task / pkill fallback (末尾「停止动作」节)
```

主仓启动直接跳到 Step 3. **worktree 内启动必须按 1 → 2 → 3 顺序**, 不能乱:
- 跳过 Step 1 → launcher 报 "config.yaml 不存在" 失败
- 跳过 Step 2 → 浏览器加载主仓代码, worktree 改动不可见, 或跨仓 API 漂移浏览器报错
- 跳过 Step 3 预检 → 跑空轮询超时浪费 60-120s

**关键决策点用 askUser 显式 gate**, 不擅自动. 涉及 cp 多个文件 / 改用户 `.env.local` / 改 `package.json` / `git reset` / 重启已在跑服务 / 升档全栈 / 替换主仓 agents 等动作前先问. 模糊信号 (用户没明说) → askUser, 别替用户判断.
...
**worktree 启动** (FX_*_DIR 指向 worktree 路径):

```bash
FX_AGENTS_DIR=<fx-data-agents worktree 绝对路径> \
FX_WEB_DIR=<fx-data-web worktree 绝对路径> \
  node ${CLAUDE_PLUGIN_ROOT}/skills/agents-launcher/dev-orchestrator.mjs \
  --workspace=ui --yes
```
...
- 不要绕过 askUser gate 自己做关键决策 — cp / 改 env / reset / 替换 agents 都先问
