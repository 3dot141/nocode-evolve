---
name: lark-wiki
description: "当用户需要管理飞书/Lark/豆包知识空间、空间成员、Wiki 节点层级、快捷方式，或在知识库中创建、移动、复制节点时使用。不负责文件上传下载或文档、表格、Base 正文数据编辑。"
---
...
- 用户要**按特定主题 / 关键词 / 内容线索查找资料并收集到知识库节点或新建知识库节点下**，必须切到 `lark-drive` Skill 的 `topic_move_collector` workflow。该 workflow 使用 Drive 全量搜索召回，再按 Wiki 目标解析、确认和移动；不要只用 Wiki 节点列表做局部遍历。
- 用户要**整理 / 盘点 / 归类 / 重构知识库、个人文档库、文档库目录或 Wiki 节点结构**，或要生成整理方案、目标目录树、移动计划时，不要只使用 Wiki 节点 API。必须切到 `lark-drive` Skill 的 `knowledge_organize` workflow；该 workflow 负责 Drive / Wiki / 个人文档库的统一入口解析、资源盘点、分类计划、写前确认和结果验证。
...
- 上传 / 下载文件到知识库节点下 → `lark-drive` Skill（`drive +upload --wiki-token`）
- 编辑文档正文内容 → `lark-doc` Skill
- 表格 / 多维表格数据操作 → `lark-sheets` / `lark-base` Skill
- 按名称搜索文档 / Wiki / 表格文件、评论与权限管理 → `lark-drive` Skill
