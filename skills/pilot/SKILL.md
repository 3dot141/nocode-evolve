---
name: pilot
description: 工程任务流程领航 (手动入口). 用户主动 /调 进入, 给"当前阶段判断 + 下一步建议 + 备选", 用户拍板, 不自动执行. agent 视角: 复杂 / 多步 / 跨阶段任务 (跨文件 + 状态未知 / 需要 commit / PR / 设计文档 / 评审 / 用户说"整个/整体/全流程") 时, 主动一句话建议用户 "这任务多步, 要不要 /pilot 看下流程?" 并停下等用户拍板, 不替用户决定. 不用于简单单步任务.
disable-model-invocation: true
---

# nocode-evolve:pilot — 工程任务流程领航 (手动入口)

> 你 **主动 `/调`** 进入的工程任务驾驶舱。**不自动触发**——你需要导航时才按它。
...
- **不自动触发**: 由 `disable-model-invocation: true` 保证 harness 不会自己加载。
