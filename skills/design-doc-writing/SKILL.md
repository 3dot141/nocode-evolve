...
- **总图**（`## 架构 → ### 架构图 / 流程图` 节）：聚焦组件关系 / 主链路，让 reviewer 30 秒 grasp 全局
- **子图**（`#### 问题 X` 节下）：单一问题如果有局部状态机 / 多分支 / 多张力同时拍板，纯文字读不下去时画

子图触发判据 + 完整规则详见 `references/doc-types/design-doc.md`《架构图 / 流程图》节。
...
- **依赖**：`→` 或 `↓`
- **时序流程**：节点 + `↓` 串联
- **粒度**：总图 组件 ≤ 7 / 流程节点 ≤ 10，超了→ 下沉到子图，不靠把总图画大
