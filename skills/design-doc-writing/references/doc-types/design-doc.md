...
## 验证策略         (Full 场景含路径: TO 表 + 测试层级分组 + 不测项 + 路径覆盖状态表; 见下)
## 其他
...
**与 devflow Define 阶段的关系**: 如果走了 devflow，Define 阶段已与用户对齐的成果定义和验收标准应直接体现在本节。本节是 Define 阶段产出的落地位置——不是独立重新想，而是把已确认的验收标准（含边界 case）写进设计文档。
...
Full 场景从 design skill 接收 verify 策略时，**必须落盘为本固定章节**（不只存在于会话文本）——Verify 阶段直接从这里读取作为执行依据。非 Full 场景或无路径建模可省此节。

路径/约束 ID 约定见 `{NOCODE_SKILL_REF}/path-conventions.md`。本节四块：

1. **测试目标表**：TO 编号 + 覆盖路径 + 覆盖约束 + 测试层级 + 说明
...
2. **按测试层级分组**：E2E / 集成 / 单测 / 手动 各覆盖哪些路径、怎么跑、mock 什么
3. **不测项**：哪些路径不测 + 具体原因（无测试环境 / 第三方沙箱不支持 / 成本不合理）+ 风险评估。不能笼统写"测不了"
4. **路径覆盖状态表**：路径 | 方案覆盖 | TO 覆盖 | verify 策略 | 状态（全 ✅ 才算完整）
