...
select 回答"走哪条路"——**先扩散再收敛**:探索 approach、多方案差异化对比,扩散结果交用户拍方向(Step 4a),再收敛下钻,产出结构化 **决策包 Decision Packet** 交给 `dev-design-refine` 写详细设计。本 skill 只做决策,不写文档、不评审。
...
沟通结构 = **1 个方向决策点(4a)+ 全程账本可见 + 1 个终审点(8a)**:扩散的结果必须交用户拍方向;收敛过程 agent 自主批处理——探索、L2/L3 下钻、失败预演、测试计划都不因"要不要问用户"而停顿,每项决策锁定即落账本。

**例行确认两处,每次必过,不是例外触发**:
- **Step 4a L1 架构确认**:首轮 2-3 方案 + 权衡表成型后、选定前,交用户拍方向。这是杠杆最大的介入点——此处否决只废一轮提案,拖到 8a 才否决要废 Step 4-8 全部工作。
- **Step 8a Packet 终审**:Packet 文件用户逐条审核编辑,改完 agent 读回应用。审的是领域决策 / TO 这层细节(方向已在 4a 定过)。

**决策账本**(非阻塞可见):Step 5 起每锁定一项决策就增量追加到 Packet 文件(条目格式见 Step 5),用户随时可打开看"定了什么、还剩什么";层级边界(L1/L2/L3 锁定、6c 完成)在回合内给 1-2 行状态注记(本轮新锁 N 项 / 待定 M 项),不要求确认、不阻塞。8a 终审因此审的是一份用户看着长出来的文件,不是陌生成品。

**异常停下来问,仅限**:① 打平手(权衡相当,取决于用户主观优先级)② 冲突需拍板(与已有 ADR/wiki 决策冲突)③ 信息缺口(需 agent 拿不到的外部信息)④ 不可逆 + 高影响。

**协调模式区分**:被 `dev-design`(协调器)调用时,遇上述异常例外返回 `needs_user_input`(由协调器统一弹确认,不自行弹);独立运行时用本地 AskUserQuestion。Step 4a / 8a 是流程内置例行确认,无论独立运行还是被协调器调用都由 select 自行执行。
...
循环:每轮聚焦一个决策层级(L1 架构 / L2 组件 / L3 细节),选定后检查有无下一层子决策,有就再开一轮。首轮(L1)选定前必过 Step 4a 用户确认;后续轮 agent 自主。
...
- **Step 4a L1 架构确认(仅首轮,例行)**:方案 + 权衡表(+ spike 结论,如做)成型后、选定前,交用户拍方向。两回合模式,方案对比禁塞 AskUserQuestion:
  - **展示回合**:方案卡片 + 逐维度权衡表 + agent 推荐及理由作为**回合末尾文本**完整输出,同时告知 Packet 文件路径(账本将落此文件);末尾只问**一个最承重的问题**——架构方向选哪个。结束回合,不接任何工具调用(方案对比塞 `question` 挤成密集段落,塞 `preview` 被终端折叠)。
  - **确认回合**:用户回应即决策(采纳推荐 / 改选备选 / 注入权重偏好)→ 直接按其执行;要求补方向 → 回 Step 3 补提再走展示回合;回应含糊才 AskUserQuestion 澄清。
  - research 模式不走 4a(预研直接交付终止,用户在交付后拍板)。
- **Step 5 方案选定 + 落账 + 层级下钻**:L1 按 4a 拍板结果选定;L2/L3 agent 按权衡表 + restate 约束自主选定(异常例外才问)。每项选定即追加**决策账本**——账本落在 Packet 文件(`{topic}-decision-packet.md`,与 `{dev_design_output}` 同目录,首轮创建),条目格式:

  `决策点 | 结论 | 依据/置信度 | 否决备选及原因 | P0 还是延后`

  账本是收敛进度的度量,也是 8a 终审文件的骨架。层级下钻检查(展开一层列子决策,跟 PRD 功能领域交叉)→ 有子决策回 Step 3,全无进 Step 6。
...
Packet 文件（`{topic}-decision-packet.md`,与 `{dev_design_output}` 同目录）已随决策账本从 Step 5 起增量生长;本步将其补全为完整 Packet 结构（markdown）交用户终审。格式要求：
...
- [ ] 方案已选定(L1 经 Step 4a 用户拍方向,L2/L3 agent 自主;理由入决策账本)
...
- [ ] Step 8a 用户终审通过（文件已随账本生长并补全 → 用户已编辑 → 变更已读回应用;方案级变更已回 Step 3 重选）
