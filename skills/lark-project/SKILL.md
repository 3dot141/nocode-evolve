...
> 3. 关联类必填字段（`workitem_related_multi_select`）值为 stringified 数字数组（如 `"[7071224906]"`）；**关联对象是业务判断**：须用户提供或从该工作项上下文查实（如同分支提交所挂需求），不得 mock。「缺陷来源于 X」类来源字段例外——按团队惯例选**标题与当前工作项相同**的候选（MQL 标题搜索近似推荐列表，无相同项才问用户；详见 meegle `sop-transition-state.md` 4.4）。
