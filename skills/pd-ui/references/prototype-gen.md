...
- **拆分（独立页面文件）**：每个页面一个文件，独立审查每页的视觉设计。低保真到这里就够了
...
高保真的解法：独立页面文件保留不动，**额外创建一个组合文件**（`prototype.html`），把所有页面的代码融合进来，用 tab/section 切换模拟导航。单文件内 JS 正常执行，弹窗、抽屉、状态切换都能做。
...
- library.html — 资源库：顶部筛选栏 + 数据表格 + 批量操作条
