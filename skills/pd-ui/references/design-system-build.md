...
> pd-ui Step 5 的展开 reference。设计系统 = 品牌渲染层（颜色 / 字体 / 组件），让多个页面看起来像同一个产品。本文覆盖：三步走顺序、创建方式选型、gap analysis、并行创建、DesignSync 操作参考。
...
| **`.dc.html` 手写 + DesignSync 工具推** | 兜底：无代码、又想从 Claude Code 端控制 | 手写的静态 HTML 展示卡片（见第五节格式） |
...
└─ 没有（纯产品设计阶段）
   ...
   └─ 想从 Claude Code 端逐文件控制 → .dc.html 手写 + DesignSync 工具推
```

> `/design-sync`（命令）和 `DesignSync`（内置工具）是两回事：前者推真实 React 组件 bundle，后者 CRUD `.dc.html` 文件到设计系统项目。无代码阶段用 DesignSync 工具推 `.dc.html`，有代码阶段用 `/design-sync` 推 bundle。
...
每个缺失组件 spawn 一个 subagent，并行执行。每个 subagent 只产出 `.dc.html` 写到共享 localDir，**不调 DesignSync**（推送收口在编排者，避免多 plan 竞争）。

精简版 subagent prompt 模板：

```
为 DesignSync 设计系统产出一个组件的 .dc.html 文件。只产出文件，不调 DesignSync。
...
### finalize_plan → write_files 两步事务
...
`finalize_plan` 锁定要写的路径集合，返回 `planId`；`write_files` 凭 `planId` 提交，每个 path 必须在 plan 的 writes 里。

### 并发限制（重要）

`planId` 有状态。**不要对同一 project 并发开多个 plan 写**——并发提交会争用文件索引（`@dsCard` 卡片重建），互相覆盖。

正确做法：**并行生产所有 `.dc.html`（纯本地写文件，无 API）→ 单个 plan 批量 `writes: [...]` 一次性推送**。即「并行算、批量提交」。
