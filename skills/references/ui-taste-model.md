...
没有设计参考（无 pd-vd 产出、无截图、无 Figma）时，从这里选一个视觉方向。选完后 taste model 的使命就结束——它被消化成具体的视觉值（配色/排版/间距），写进设计文档或 `.vd.md`，下游不再需要回来找它。
...
选定后 `Skill()` 加载对应 skill，它会给出具体的设计原则（配色方案、字号体系、间距规则、组件风格）。把这些值写进设计文档 / `.vd.md`，taste model 的任务就完成了。
