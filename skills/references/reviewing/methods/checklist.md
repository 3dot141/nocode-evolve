...
- 每条 finding：`axis` = 维度名（来自领域维度表）；`location` 必填到 `file:line` / `[锚点]`；`severity` = critical / warning / suggestion；`source` = `蓝军` 或方法名（自评型）/`subagent`/`红军(Codex)`（异源交叉时）。
...
| **异源交叉**（重档 / 代码 diff / 设计文档） | 是 | 是 | 清单同时交独立 reviewer + codex 跑，**CLAIM 剥离**后合并——同位置同维度的交集 = 高置信 |

档位：
- 轻档 / 低风险 → 自评清单单跑即可
- 重档（代码 diff / 设计文档 / 安全）→ 叠加异源交叉（见 §4.3 选择表「checklist + red-blue 异源交叉」），独立性提升

异源交叉的 CLAIM 剥离 / codex 降级走框架公共能力（skeleton 步骤 5）：codex 不可用 → 单 subagent + 明说降级。
