...
**选 A**。当前单实例部署，EventEmitter 零依赖、零延迟。预留接口，多实例化时替换为 Redis（实现 `EventBus` 接口即可，调用方不变）。
...
**选 B**。事件驱动有"漏发"风险（比如进程重启时内存事件丢失）。保留一个 5 分钟低频轮询作为兜底网，确认事件机制稳定后再决定是否移除（见迁移策略 Step 3）。
...
> 重构特别要监控"新旧机制的此消彼长"——事件路径是否真的接管了，兜底是否在空转。
