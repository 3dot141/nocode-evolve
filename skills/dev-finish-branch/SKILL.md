...
- 用户说「PR 合了 / 流转任务 / 合并后流转 / 任务状态改一下」（见 `references/post-merge.md`）
...
**决策线（进入时一次确认）**：选定 disposition 后，reference 开头先展该路径的**完整决策线**，用户一次确认整条（含合并后清 worktree + 流转的目标状态）；确认后中途只保留安全 Gate（Title-Body / PR / force）。
...
**Gate 展示总则（覆盖下表全部 Gate）**：确认内容须自足内嵌 AskUserQuestion payload——短字段（target / title / 一行值）写进 `question` 文本（写**实际值**，不是字段名），长内容（body / Affected 路径列表 / 计划）放 `options[].preview`（各选项同一份，等宽多行 markdown，单选 Gate 均适用）。**禁止**「以上 / 上述…确认?」这类指代前文问法——Gate 前刚跑完 Bash 时，工具调用间自由文本 harness 不保证渲染、常被吞，用户看不到待确认内容（生产实证）。自由文本可冗余照发，但 payload 缺内容即违规。超长内容（数百行级，塞不进 preview）降级为纯文本 Gate：内容作为**回合末尾**文本发出后结束回合等用户打字回复，绝不「文本展示 + 同回合 ask」。（插件级总则见 `model/agent-about.md`「常驻交互习惯」）
...
> `run_in_background` 会话级机制；兜底见下方 Fallback。
...
- Option 3 (Keep) → 不清理
- 清理顺序：确认 merge/discard 成功 → `cd "$MAIN_ROOT"`（worktree 内跑会静默失败）→ `git worktree remove <path>` → `git worktree prune`；remove 须早于删 branch，顺序反了 `branch -d` 会拒绝（worktree 还在引用）
- 未提交改动 → remove 报错，不加 `--force`，用户先 stash

## 不要

- 不要创建时塞 reviewer（`gh pr create --reviewer` 或 `reviewers` 数组）— 单 user 错（大小写/无权限）会让整个 PR 建不出来，拆 create + edit
