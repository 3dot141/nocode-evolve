...
进入 Build 时 Read `{NOCODE_SKILL_REF}/design-traceability.md` 与 `{NOCODE_SKILL_REF}/source-comment-contract.md`。Plan 的 `designRevision` / `designDigest` 是不可变基线，`designCovers` 是已确认 task scope 的一部分：执行方只能实现并回报，不能自行更换基线或 Design ID。
