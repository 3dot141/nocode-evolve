...
| 文件 | 作用 |
|---|---|
| `hooks.json` | Hook 注册源：SessionStart、PreToolUse、PostToolUse、Stop。SessionStart 首项通过 `session-open.mjs` 建立 session-scoped state 并注入 `NOCODE_SESSION_ID`；compiler 按平台改写 codec/runtime entry，并依据 `plugin/exclusions.json` 移除 Continuous Learning observer。 |
...
- SessionStart 通过本平台 runtime-state provider 初始化 session；不注册通用 Read usage tracker 或 Stop guard。
...
- SessionStart 通过 Codex runtime entry 和本平台 runtime-state provider 初始化 session；不注册通用 Read usage tracker 或 Stop guard。
...
测试覆盖领域判断、两个 codec、domain registry/contract、session/handoff/plan state、显式 Wiki usage、Open Design fallback、Continuous Learning exclusion、确定性编译与 SessionStart/PreToolUse/PostToolUse/Stop 回归行为。
