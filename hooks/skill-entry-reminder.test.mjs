...
test('skill 在名单 + 主 agent (无 agent_id) → 返回 inject, 含关键词, 无 permissionDecision', () => {
  const out = decideReminder({ tool_name: 'Skill', tool_input: { skill: 'nocode:dev-plan' } }, SKILL_LIST);
...
test('skill 不在名单 → null', () => {
  assert.equal(decideReminder({ tool_name: 'Skill', tool_input: { skill: 'nocode:bkt' } }, SKILL_LIST), null);
});
...
test('agent_id 存在 (fork) → null, 即便 skill 在名单', () => {
  assert.equal(decideReminder({ agent_id: 'sub-123', tool_name: 'Skill', tool_input: { skill: 'nocode:dev-plan' } }, SKILL_LIST), null);
});
...
test('空名单 → 一律 null (loadSkillList 读不到时的退化)', () => {
  assert.equal(decideReminder({ tool_name: 'Skill', tool_input: { skill: 'nocode:dev-plan' } }, []), null);
});
